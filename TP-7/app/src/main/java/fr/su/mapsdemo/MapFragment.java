...
/**
 * Created by Jean-Noel on 8/11/15.
 */
public class MapFragment extends SupportMapFragment implements GoogleApiClient.ConnectionCallbacks,
    ...
    public void onMapLongClick(LatLng latLng) {
        ...
        options.icon( BitmapDescriptorFactory.fromBitmap(
                BitmapFactory.decodeResource(getResources(), fr.su.mapsdemo.R.mipmap.ic_launcher ) ) );
    ...
    private void drawCircle( LatLng location ) {
        ...
        //Radius in meters
        options.radius( 10 );
        options.fillColor( getResources().getColor( fr.su.mapsdemo.R.color.fill_color ) );
        options.strokeColor( getResources().getColor( fr.su.mapsdemo.R.color.stroke_color ) );
    ...
    private void drawOverlay( LatLng location, int width, int height ) {
        ...
        options.image( BitmapDescriptorFactory
                .fromBitmap( BitmapFactory
                    .decodeResource( getResources(), fr.su.mapsdemo.R.mipmap.ic_launcher ) ) );
    ...
    public void onCreateOptionsMenu(
        ...
        inflater.inflate(fr.su.mapsdemo.R.menu.menu_main, menu);
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case fr.su.mapsdemo.R.id.action_clear: {
                getMap().clear();
                return true;
            }
            case fr.su.mapsdemo.R.id.action_circle: {
                drawCircle( new LatLng( mCurrentLocation.getLatitude(), mCurrentLocation.getLongitude() ) );
                return true;
            }
            case fr.su.mapsdemo.R.id.action_polygon: {
                drawPolygon( new LatLng( mCurrentLocation.getLatitude(), mCurrentLocation.getLongitude() ) );
                return true;
            }
            case fr.su.mapsdemo.R.id.action_overlay: {
                drawOverlay( new LatLng( mCurrentLocation.getLatitude(), mCurrentLocation.getLongitude() ), 250, 250 );
                return true;
            }
            case fr.su.mapsdemo.R.id.action_traffic: {
                toggleTraffic();
                return true;
            }
            case fr.su.mapsdemo.R.id.action_cycle_map_type: {
                cycleMapType();
                return true;
            }
            default:
